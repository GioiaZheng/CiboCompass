...
func (app *application) dishcommentHandler(response http.ResponseWriter, request *http.Request) {
	dishName, err := app.ReadDishNameParam(request)
	if err != nil {
		app.notFoundResponse(response)
		return
	...
	comment, err := app.ReadDishComment(request)
	if err != nil {
		app.badRequestResponse(response, err.Error())
		return
	}
	
	err = database.AddDishComment(app.db, app.logger, dishName, comment)
	if err != nil {
		if err == database.ErrDishNotFound {
			app.notFoundResponse(response)
		} else {
